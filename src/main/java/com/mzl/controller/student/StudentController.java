...
public class StudentController {
    ...
    public String changePass(@RequestParam("prepass") String prepass, @RequestParam("nowpass") String nowpass, Model model, HttpServletRequest request){
    ...
    public String courseDetail(@Param("classId") int classId, Model model, HttpServletRequest request){
        if (courseService.checkStuIns(classId, (Integer) request.getSession().getAttribute("stuid"))){
            model.addAttribute("course", courseService.queryCourse(classId));
            return "student/courseDetail";
        }else {
            model.addAttribute("msg", "请注意课程的学院限制");
            model.addAttribute("paging", pageService.subList(1, courseService.queryAllCourse((Integer) request.getSession().getAttribute("stuid"))));
            model.addAttribute("teaList", userService.queryAllTeacher());
            model.addAttribute("insList", courseService.queryAllIns());
            return "student/courseList";
        }
    ...
    public String deleteCourse(@Param("courseid") int courseid, Model model, HttpServletRequest request){
        courseService.deleteCourseChoose((Integer) request.getSession().getAttribute("stuid"), courseid);
    ...
    public String checkedCourseList(Model model, HttpServletRequest request){
        model.addAttribute("courseList", courseService.queryStuCourse((Integer)request.getSession().getAttribute("stuid")));
